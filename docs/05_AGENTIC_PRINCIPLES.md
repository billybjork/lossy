...
**Frame Capture Triggers (Target for Sprint 11+):**
```javascript
...
> **Implementation Roadmap:** Always-on observation is being introduced in phases. Sprint 10 delivers passive audio triggers and a debug view while preserving the current per-recording backend. Sprint 11 extends the lifecycle to continuous sessions and frame sync. Diffusion-style refinement lands after persistent review state is in place. Each phase should be feature-gated so manual controls remain available during rollout.
...
**Next:** See `docs/sprints/SPRINT_10_always_on_agent.md` for implementation roadmap.
