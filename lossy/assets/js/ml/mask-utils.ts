...
/**
 * Represents a contour (outline) of a mask region
 */
 ...
 *
 * Used for rendering marching ants animation on pending masks.
 */
export function findAllContours(imageData: ImageData): Array<Contour> {
  return findAllContoursWithHoles(imageData);
}

/**
 * Marching squares contour tracing implementation.
 * Traces both outer contours and holes, classifying them by signed area (winding).
 */
export function findAllContoursWithHoles(imageData: ImageData): Array<Contour> {
  const { width, height, data } = imageData;
