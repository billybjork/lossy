...
export class VideoDetector {
  ...
  async detect() {
    ...
    if (this.videoElements.length === 0) {
      console.log('[VideoDetector] No video elements found, waiting...');
      return this.waitForVideo();
    }

    this.primaryVideo = this.selectPrimaryVideo(this.videoElements);
    console.log('[VideoDetector] Primary video selected:', this.primaryVideo);
  ...
  selectPrimaryVideo(videos) {
    ...
    console.log('[VideoDetector] Multiple videos found:', videos.length);
    ...
    console.log('[VideoDetector] Video scores:', scored.map(s => s.score));
  ...
  watchForChanges(callback) {
    this.observer = new MutationObserver((mutations) => {
      ...
      if (!currentVideos.includes(this.primaryVideo)) {
        console.log('[VideoDetector] Primary video removed, re-detecting...');
      ...
      for (const mutation of mutations) {
        if (mutation.type === 'attributes' && mutation.target === this.primaryVideo) {
          if (mutation.attributeName === 'src' || mutation.attributeName === 'currentSrc') {
            console.log('[VideoDetector] Video src changed, notifying...');
            callback(this.primaryVideo);
            return;
          }
        }
    ...
    console.log('[VideoDetector] Watching for video changes (DOM + attributes)');
  }
